...
//////////////////////////////////DO NOT MODIFY BELOW////////////////////////////////////////////
	while {true} do {
		if (({_x in (uniformItems player)} count _restrictedItems) > 0) then {
			...
			titleText ["That item is BANNED!", "PLAIN", 3]; // The comment it tells the player
			titleFadeOut 1; 
		};
		...
		if (({_x in (weapons player)} count _restrictedItems) > 0) then {
			...
			titleText ["That weapon is BANNED!", "PLAIN", 3]; // The comment it tells the player
			titleFadeOut 1; 
		};
		if (({_x in (primaryWeaponItems player)} count _restrictedItems) > 0) then {
			...
			titleText ["That scope is BANNED!", "PLAIN", 3]; // The comment it tells the player
			titleFadeOut 1; 
		};
		if((_restrictedItems find (backpack player)) >= 0) then {
			...
			titleText ["That is BANNED!.", "PLAIN", 3]; // The comment it tells the player
			titleFadeOut 1; 
		};
		sleep 30;
	};
