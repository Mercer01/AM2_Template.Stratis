...
_autoRiflemen = [ 	//The slots that can use the weapons
	"B_Officer_F",
	"B_soldier_AR_F",
...
while {true} do {
	while {_safeZone distance player < _dis} do {
		...
		if (({_x in (primaryWeaponMagazine player)} count _mag) > 0) then {
			if (({player isKindOf _x} count _autoRiflemen) < 1) then {
				{player removeWeapon _x;} count _mag; 
				player addPrimaryWeaponItem "rhs_mag_30Rnd_556x45_M855A1_Stanag"; //Mag it's gonna get replaced with
				titleText ["That's too heavy for you!", "PLAIN", 3]; // The comment it tells the player
				titleFadeOut 1; 
			};
			sleep 10;
		};
